...
RCT_EXPORT_METHOD(deleteUserData:(RCTPromiseResolveBlock)resolve
                  reject:(RCTPromiseRejectBlock)reject) {
    if (!_initialized) {
        resolve(@"{\"success\":false,\"error\":\"not_initialized\"}");
        return;
    }

    [_queue addOperationWithBlock:^{
        // POST deletion request to server
        NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/privacy/delete", self->_baseUrl]];
        ...
        body[@"app_key"] = self->_appKey;
        body[@"install_uuid"] = self->_installUuid;
        if (self->_userId) body[@"user_id"] = self->_userId;
        ...
        // Clear local state
        self->_userId = nil;
        self->_userProperties = nil;

        NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
        [defaults removeObjectForKey:kPrefsInstallUuid];
        [defaults removeObjectForKey:kPrefsAttribution];
        [defaults synchronize];

        // Regenerate install UUID
        self->_installUuid = [self getOrCreateInstallUuid];

        BOOL ok = statusCode >= 200 && statusCode < 300;
        resolve([NSString stringWithFormat:@"{\"success\":%@}", ok ? @"true" : @"false"]);
    }];
}
